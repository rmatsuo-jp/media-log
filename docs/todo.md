...
- **外部API連携のアーキテクチャ改善**（Annict連携・NDL典拠IDによる名寄せ・Firestoreキャッシュ・複数API結果の類似度スコアリング）: 詳細・背景は[external-media-integration.md#アーキテクチャ評価と改善方向](external-media-integration.md#アーキテクチャ評価と改善方向)を参照。
