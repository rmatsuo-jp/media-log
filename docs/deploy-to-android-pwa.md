...
{
  "name": "Media Log",
  "short_name": "Media Log",
  "description": "マンガ・書籍・アニメ・映画の閲覧記録を一元管理するアプリ",
  "display": "standalone",
  "scope": "./",
  "start_url": "./",
...
```powershell
ng build --configuration production --base-href "https://あなたのGitHubユーザー名.github.io/media-log/"
```

**このアプリの場合：**

```powershell
ng build --configuration production --base-href "https://rmatsuo-jp.github.io/media-log/"
```
...
**⚠️ base-hrefの注意点：**
`--base-href=/media-log/` のようにパスだけを指定すると、Windows環境では `C:/Program Files/Git/media-log/` という**ローカルのファイルパスに誤解釈されることがあります**。
必ず `https://` から始まる完全なURLを指定してください。

ビルド結果は `dist/media-log/browser/` に生成されます。
ビルド後に `dist/media-log/browser/index.html` を開き、以下のようになっているか確認してください：

```html
<base href="https://rmatsuo-jp.github.io/media-log/" />
```
...
```powershell
npx angular-cli-ghpages --dir=dist/media-log/browser
```

このコマンドは以下を自動で行います：

1. `gh-pages` ブランチを作成または更新する
2. `dist/media-log/browser/` の中身を全てそのブランチにpushする
...
1. Pixel の Chrome で `https://rmatsuo-jp.github.io/media-log/` を開く
2. アドレスバーの右側にインストールアイコン（⬇️）が表示されたらタップ
   - または右上の「︙」メニュー → **「アプリをインストール」**
3. 「インストール」をタップ
4. ホーム画面に「Media Log」アプリが追加される
...
| 他の端末から見えるか     | 見えない                                             |
| サーバーに送られるか     | 送られない（クラウド同期時のFirebaseへの送信を除く） |
...
→ `dist/media-log/browser/index.html` の `<base href>` を確認してください。
