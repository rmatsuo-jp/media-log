...
export class MangadexApiService {
  ...
  getVolumes(mangaId: string): Observable<ExternalUnitCandidate[]> {
    ...
    return this.http.get<MangaDexCoverResponse>(`${MANGADEX_API}/cover?${params}`).pipe(
      map((res) =>
        res.data
          .filter((entry) => entry.attributes.volume != null && entry.attributes.volume !== '')
          .map((entry) => ({
            number: Number(entry.attributes.volume),
            coverImageUrl: coverUrl(mangaId, entry.attributes.fileName),
          }))
